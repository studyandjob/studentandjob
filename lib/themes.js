...
export const DEFAULT_THEME_ID = 'modern-sky-blue-teal';
...
export const THEMES = [
  // New default (Sept 2026): the "Online Jobs & Study" reference design —
  // a clean SaaS-style blue primary (header links, buttons, hero badge)
  // with a teal/emerald accent (study-zone icons, secondary CTAs) on a
  // very light blue-white page background. Matches the client-supplied
  // homepage mockup 1:1 so nothing needs per-component color overrides.
  {
    id: 'modern-sky-blue-teal',
    name: 'Sky Blue & Teal (Default)',
    isDefault: true,
    primary: '#155DFC',
    accent: '#0D9488',
    background: '#F5F8FE',
  ...
  // clean white surfaces. Set as the new default theme.
  {
    ...
    // Renamed to match the new default colors below (id kept the same on
    // purpose — it's what's stored in site_settings.active_theme, so
    // renaming/recoloring this entry is what makes it a true "upgrade"
    // for every visitor already on the default theme, without breaking
    // anyone's saved selection).
    name: 'Deep Teal & Gold',
    // Previous default was a generic SaaS-dashboard green+blue pairing.
    // This swaps in a deep teal (credibility, official documents) with a
    // warm gold accent (certificates/seals) on a warm paper-toned
    // background instead of a cold blue-gray — reads as an education/
    // government-services portal rather than a generic admin dashboard.
    primary: '#0F6B5C',
    accent: '#B8860B',
    background: '#F7F5EF',
    text: '#1E2A28',
  ...
  // primary like "atl2" (buttons/nav) and a gold accent like the fixed
  // "agold" (#8A5A17, chosen for AA-contrast + a Pakistan flag-style
  // green+gold pairing) instead of the coral badge color, since coral/red
  // is already used site-wide as the "Expired / Urgent" danger signal and
  // would visually clash if reused as a brand accent.
  {
    id: 'admin-panel-match',
    name: 'Admin Panel Match (Green & Gold)',
    primary: '#1E8449',
    accent: '#8A5A17',
    background: '#F1F4F8',
    text: '#1F2937',
