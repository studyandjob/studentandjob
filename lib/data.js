...
export async function getHeroSlides() {
  ...
  // Defensive: next/image throws if src is empty/missing, which would take
  // down the whole homepage's build if a slide was ever saved without an
  // image or video (e.g. a row created mid-upload). Silently skip those
  // rather than letting one bad row break the page for everyone. A slide
  // is valid if it has EITHER an image_url OR a video_url set.
  return (data || []).filter((slide) => {
    const hasImage = typeof slide.image_url === 'string' && slide.image_url.trim() !== '';
    const hasVideo = typeof slide.video_url === 'string' && slide.video_url.trim() !== '';
